...
function LessonsPage({user}) {
    ...
    return (
        /* BACKGROUND FIX: Use var(--bg-main) and var(--text-main) */
        <div className="relative w-full min-h-screen p-8 md:p-16 transition-colors duration-500"
             style={{ backgroundColor: "var(--bg-main)", color: "var(--text-main)" }}>
            
            {/* Background Glows: Use var(--brand-gold) */}
            <div className="fixed top-[-10%] left-[-10%] w-[600px] h-[600px] blur-[130px] rounded-full pointer-events-none opacity-10" 
                 style={{ backgroundColor: "var(--brand-gold)" }} />
            <div className="fixed bottom-[-10%] right-[-10%] w-[500px] h-[500px] blur-[120px] rounded-full pointer-events-none opacity-5" 
                 style={{ backgroundColor: "var(--brand-gold)" }} />

            <h2 className="relative z-10 text-5xl font-black mb-16 tracking-tighter ml-4"
            ...
            <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-12 w-full">
                {lessonCategories.map((category, index) => {
                    ...
                    return (
                        <div 
                            key={category.id} 
                            className={`group transition-all duration-500 hover:translate-y-[-8px] flex flex-col justify-between backdrop-blur-xl rounded-[60px] p-10 border shadow-2xl ${isCategoryComplete ? 'border-[var(--brand-gold)]' : 'border-[var(--border-color)]'}`}
                            style={{ 
                              backgroundColor: "var(--bg-card)", 
                              borderColor: isCategoryComplete ? "var(--brand-gold)" : "var(--border-color)",
                            ...
                            <div className="space-y-6">
                                <div className="flex items-start justify-between">
                                    ...
                                    {isCategoryComplete && (
                                        <Badge className="font-bold rounded-full px-3 py-1"
                            ...
                            <div className="mt-10 space-y-4">
                                <div className="h-6 flex justify-center">
                                    {isComboComplete && (
                                        <div className="flex items-center text-xs font-bold tracking-widest uppercase animate-pulse"
                                             style={{ color: "var(--brand-gold)" }}>
                                ...
                                <Button asChild className="w-full font-black py-8 rounded-full text-lg uppercase tracking-wider transition-all active:scale-95 shadow-lg"
                                        style={{ backgroundColor: "var(--brand-gold)", color: "#000" }}>
