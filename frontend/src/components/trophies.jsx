...
// 1. THE GRID WRAPPER
// This is what stops the "cut-off" bug.
export function TrophiesCard({ trophies }) {
  return (
    <div className="w-full py-[1rem]">
      {/* The Magic Line: 
        grid-cols-[repeat(auto-fit,minmax(min(100%,280px),1fr))]
        It tells the cards: "Try to stay 280px wide. If you can't fit 3 anymore, 
        drop the 3rd one to a new row automatically."
      */}
...
// 2. THE INDIVIDUAL TROPHY
function Trophy({ trophy }) {
  const isUnlocked = trophy.unlocked;

  return (
    <div 
      /* suppressHydrationWarning is the shield against the removeChild crash */
    ...
    >
      ...
      {/* Icon Container - Uses 'rem' so it shrinks/grows with your settings */}
      <div className={cn(
